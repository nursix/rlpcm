...
#
# RLPCM Template Version 1.0.1 => 1.1.0
#
# Execute in web2py folder after code upgrade like:
# python web2py.py -S eden -M -R applications/eden/modules/templates/BRCMS/RLP/upgrade/1.0.1-1.1.0.py
#
import sys
...
TEMPLATE_FOLDER = os.path.join(request.folder, "modules", "templates", "BRCMS", "RLP")
...
if not failed:
    ...
    auth.permission.delete_acl("AUTHENTICATED", c="br")
...
if not failed:
    ...
    def cms_post_duplicate(item):
        ...
        if name in ("RegistrationOrgIntroDisabled",
                    "OrgSelfPresentationIntro",
                    "Subject WelcomeProvider",
                    "Message WelcomeProvider",
                    ):
...
#
if not failed:
    info("Replace default organisation types")
